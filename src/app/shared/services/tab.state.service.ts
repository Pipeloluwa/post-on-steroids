...
export class TabStateService {
    ...
    async shareCapsule(capsuleId: string): Promise<string> {
        ...
        if (res?.data) {
            ...
            const origin = isFile ? 'http://localhost:5000' : window.location.origin;
            return `${origin}${isFile ? '/#/' : '/'}import?share=${res.data.shareToken}`;
        }
